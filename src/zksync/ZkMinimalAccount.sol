...
contract ZkMinimalAccount is IAccount, Ownable {
    ...
    function _validateTransaction(
        ...
        SystemContractsCaller.systemCallWithPropagatedRevert( //this is a sort of simulation and it will only run if in the toml we have is system = true
            uint32(gasleft()),
            address(NONCE_HOLDER_SYSTEM_CONTRACT), // this contract hold all the nonces via mappping
            0,
            abi.encodeCall(
                INonceHolder.incrementMinNonceIfEquals, //here we increment the nonce
                (_transaction.nonce)
            )
        );
